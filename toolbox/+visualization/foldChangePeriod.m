function output = foldChangePeriod(dataCycle, varargin)
...
req = ["TRACK_ID","START_INDEX","CYCLE_ID","DURATION"];
...
for i = 1:numel(ids)
    % disp(i);
    tmpPeriods = T.DURATION(grp==ids(i));
    if length(tmpPeriods) > 1
        fc = [1; tmpPeriods(2:end)./tmpPeriods(1:end-1)];
        T.FC_Period(grp==ids(i)) = fc;
    else
        T.FC_Period(grp==ids(i)) = 1;
    end
end
